			...
			if($image_size < 1000000){
				if($image_ext == 'jpg' || $image_ext == 'png' || $image_ext == 'gif'){
					if(move_uploaded_file($image_tmp,$image_path)){
						$up_sql = "UPDATE posts SET post_title='$title', post_description='$_POST[description]', image='$image_db_path', post_category='$_POST[category]', product_status='$_POST[status]' WHERE post_id = '$_POST[post_id]'";
						...
						<select id="category" name="category" class="form-control" required>
							...
							<?php
								...
								while($c_rows = mysqli_fetch_assoc($run_sql)){
									if($c_rows['cat_name'] == 'home'){
										continue;
									}
									if($c_rows['cat_name'] == $rows['category']){
